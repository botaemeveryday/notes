---
title: "Программирование на C++"
description: "Хвастунов Александр Павлович"
cover: "cover.png"
semester: 2
---

Конспекты содержат немалое количество ошибок, которые были обнаружены во время подготовки к экзаменам, но не были исправлены в этих версиях, а исправленные версии утеряны... Так что повторно проверяйте сами всё... 

Если есть желание, можете помочь сделать конспекты еще лучше, отправив нам [issue](https://github.com/botaemeveryday/notes/issues/new?template=content-error.md) или [pull request](https://github.com/botaemeveryday/notes/pulls) с изменениями